...
public class Main {
   ...
   */
    ...
    public static void main(String[] args) throws Exception {
        ...
        Properties props = PropertyUtil.loadProps(projectDir + "res/virtuepay.properties");
        if (props.containsKey("api_base")) {
            VirtuePay.setApiBase(props.getProperty("api_base"));
        }

        // 设置 API Key
        VirtuePay.apiKey = props.getProperty("api_key");
        VirtuePay.appId = props.getProperty("app_id");

        // 设置私钥，用于请求签名
        VirtuePay.privateKey = FileUtil.getStringFromFile(Main.projectDir + props.getProperty("private_key_path"));

        // Payment 示例
        PaymentExample.runDemos(VirtuePay.appId);
    }
